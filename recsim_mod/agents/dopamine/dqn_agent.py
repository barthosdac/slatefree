...
from recsim_mod.agents.dopamine import dqn_agent_original
...
def recsim_dqn_network(user, doc, scope,size):
  inputs = tf.concat([user, doc], axis=1)
  with tf.compat.v1.variable_scope(scope, reuse=tf.AUTO_REUSE) :
    hidden = tf.keras.layers.Dense(size, activation=tf.nn.relu)(inputs)
    hidden = tf.keras.layers.Dense(16, activation=tf.nn.relu)(hidden)
    q_value = tf.keras.layers.Dense(1, name='output')(hidden)


    scope = tf.compat.v1.get_default_graph().get_name_scope()
    print('s',scope)
    w=tf.compat.v1.get_collection(
...
class DQNAgentRecSim(dqn_agent_original.DQNAgent):
  ...
  def __init__(self, sess, observation_space, num_actions, stack_size,
    ...
    dqn_agent_original.DQNAgent.__init__(
        self,
        sess,
        num_actions,
