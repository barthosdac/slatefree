...
def user3(Y, doc_obs) :
  for doc in doc_obs :
    cluster_id=np.argmax(doc)
    if cluster_id in Y :
      return np.random.randint(len(doc_obs))
  return None
...
class UtilityModelVideoSampler(document.AbstractDocumentSampler):
  ...
  def __init__(self,
    ...
    self.cluster_means = 3 + np.concatenate((trashy, nutritious))
...
class IEvUserDistributionSampler(user.AbstractUserSampler):
  ...
  def sample_user(self):
    ...
    temp = np.arange(NB_FEATURES)
    np.random.shuffle(temp)
    features['Y'] = [int(temp[k]) for k in range(self.get_user_ctor().NUM_FEATURES)]
...
class IEvUserModel(user.AbstractUserModel):
  ...
  def soft_reset(self) :
    self._user_state.time_budget=self._time_budget_t0
  ...
  def update_state(self, slate_documents, responses):
    ...
    for doc, response in zip(slate_documents, responses):
      if response.clicked:
        ...
        if self._implicit :## Update budget
          received_utility = (
                  user_state.document_quality_factor * (doc.quality - 3))
  ...
  def simulate_response(self, documents):
    ...
    selected_index = self.choice_fct(self._user_state.Y, doc_obs)
    ...
    if selected_index is None:
      #self._user_state.time_budget = 0
      return responses
...
def create_environment(env_config):
  ...
  if env_config['user'] == 'user1' :
    user=user1
  elif env_config['user'] == 'user2' :
    user=user2
  else :
    user=user3
