...
class IEvVideoSampler_none(document.AbstractDocumentSampler):
  ...
  def __init__(self,
    ...
    """
    super(IEvVideoSampler_none, self).__init__(doc_ctor, **kwargs)
    self._doc_count = 0
...
class UtilityModelVideoSampler(document.AbstractDocumentSampler):
  ...
  def sample_document(self):
    ...
    if self._onehot :
      ...
      features = np.random.random(self._num_clusters)
    ...
    if self._onehot :
      ...
      quality_mean = (self.cluster_means * features).sum() / features.sum()
    # Variance fixed
...
class IEvUserDistributionSampler(user.AbstractUserSampler): #UNUSED
...
def create_environment(env_config):
  ...
  document_sampler = UtilityModelVideoSampler(
      doc_ctor=IEvVideo, seed=env_config['seed'],onehot=env_config['onehot'])
